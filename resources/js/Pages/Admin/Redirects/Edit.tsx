...
export default function Edit({ redirect }: { redirect: RedirectRow }) {
    ...
    const submit = (e: FormEvent) => { e.preventDefault(); put(`/admin/redirects/${redirect.id}`, { preserveScroll: true }); };

    return (
        <AdminLayout header={<h2 className="text-xl font-semibold text-slate-800">Edit redirect</h2>}>
            ...
            <form onSubmit={submit} className="max-w-xl space-y-4 rounded-lg border bg-white p-6">
                <div>
                    ...
                    <input className={inputClass(errors.from_path)} value={data.from_path} onChange={(e) => setData('from_path', e.target.value)} />
                ...
                <div>
                    ...
                    <input className={inputClass(errors.to_path)} value={data.to_path} onChange={(e) => setData('to_path', e.target.value)} />
